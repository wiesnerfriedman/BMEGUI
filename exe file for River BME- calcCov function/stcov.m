...
nMSi=size(Zi,1);
nMEi=size(Zi,2);
if size(cMSi,1)~=nMSi | size(cMSi,2)>4
  error('cMSi must be a nMSi by 2 matrix or nMSi by 4 matrix'); 
end;
...
nMSj=size(Zj,1);
nMEj=size(Zj,2);
if size(cMSj,1)~=nMSj | size(cMSj,2)>4
  error('cMSj must be a nMSj by 2 matrix or nMSj by 4 matrix'); 
end;
...
%riverTopology = dist{2};
%[rD]=coord2distRiver(cMSi,cMSj,riverTopology)
%
