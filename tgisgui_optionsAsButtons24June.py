...
class TgisGui:
    ...
    def createScreen1(self):
        ...
        try:
            ...
            self.menubar2 = self.windowTree1.get_widget('menubar2')
            #self.menuitem11 = self.windowTree1.get_widget('menuitem11')
            #self.menuitem11.connect('activate',self.goBox2)
            self.color1 = self.windowTree1.get_widget('color1')
            self.jet2 = self.windowTree1.get_widget('jet2')
            self.jet2.connect('activate',self.goBox2)

            self.hsv2 = self.windowTree1.get_widget('hsv2')
            self.hsv2.connect('activate',self.goBox2)
            
            self.hot2 = self.windowTree1.get_widget('hot2')
            self.hot2.connect('activate',self.goBox2)

            self.cool2 = self.windowTree1.get_widget('cool2')
            self.cool2.connect('activate',self.goBox2)

            self.spring2 = self.windowTree1.get_widget('spring2')
            self.spring2.connect('activate',self.goBox2)

            self.summer2 = self.windowTree1.get_widget('summer2')
            self.summer2.connect('activate',self.goBox2)

            self.gray2 = self.windowTree1.get_widget('gray2')
            self.gray2.connect('activate',self.goBox2)


            #----------- Grid (Yes/No) on the map --------
            self.yes2 = self.windowTree1.get_widget('yes2')
            self.yes2.connect('activate',self.goBox2)

            self.no2 = self.windowTree1.get_widget('no2')
            self.no2.connect('activate',self.goBox2)


            #----------- Grid (Yes/No) on the map --------
            self.mbegui_manual1 = self.windowTree1.get_widget('mbegui_manual1')
            self.OpenManual = os.startfile('Matplotlib.pdf')
            #self.mbegui_manual1.connect('activate',self.OpenManual)

            self.bmegui_home_page1 = self.windowTree1.get_widget('bmegui_home_page1')
            self.bmegui_home_page1.connect('activate',self.goBox2)

            self.bmegui_contact1 = self.windowTree1.get_widget('bmegui_contact1')
            self.bmegui_contact1.connect('activate',self.goBox2)
 ...
 ###############################################################################           
    ...
    def showBMEMap(self,widget):
        ...
        try:
            ...
            # Plot map
            estmap = plotBMEMap.pcolor(meshX,meshY,meshMean,shading='interp',\
                                       vmin=minVal,vmax=maxVal)
            ...
            # Plot map
            varmap = plotBMEErr.pcolor(meshX,meshY,meshVar,shading='interp',\
                                       vmin=minVal,vmax=maxVal)
            ...
            if strPageTab == 'Map List':
                ...
                if flgMeanTab:
                    if int(plotId) in self.plotedNaNId_M:
                        ...
                        # Plot map
                        estmap = plotObj.pcolor(meshX,meshY,meshMean,shading='interp',\
                                                   vmin=minVal,vmax=maxVal)
                        #--------------------------P Jat (June, 2011)------------------------
                        #if os.path.exists('maskX.txt'):
                        if self.incMask ==1:
                    ...
                    if int(plotId) in self.plotedNaNId_V:
                        ...
                        varmap = plotObj.pcolor(meshX,meshY,meshVar,shading='interp',\
                                                   vmin=minVal,vmax=maxVal)
                        #--------------------------P Jat (June, 2011)------------------------
                        #if os.path.exists('maskX.txt'):
                        if self.incMask ==1:
