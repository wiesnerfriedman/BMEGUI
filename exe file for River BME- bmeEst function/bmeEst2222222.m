...
% % %     %----------- End; Original Code ---------------------------------------    
    ...
    %cs3 = rawCoord(rawType == 2,:); %--------- P Jat (Triangular pdf data)
    %cs4 = rawCoord(rawType == 2,:); %----------p Jat (Truncated Gaussian pdf)
	...
	% Calculate BME momemnts
     ...
     if (existID2==2)
          ...
          disp('Yes3');  
          save chC
          [moments,info]=BMEprobaMomentsRiver(sk,chC,csC,zhC,stypeC,nlC,limiC,...
              probdensC,covmodel,covparam,nhmax,nsmax,dmax,order,dist);
          disp('Yes4');
     else
          [moments,info]=BMEprobaMomentsRiver(sk,chC,csC,zhC,stypeC,nlC,limiC,...
              probdensC,covmodel,covparam,nhmax,nsmax,dmax,order); % only 14 input arguments
     end
