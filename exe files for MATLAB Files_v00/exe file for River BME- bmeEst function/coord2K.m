...
if (n1==0)|(n2==0),
  ...
  if ~isST,
    switch char(dist{1})
      case 'coord2dist'
        D=eval([char(dist{1}),'(c1{1},c2{1})']);
      case 'coord2distRiver'
        D=eval([char(dist{1}),'(c1{1}(:,1:4),c2{1}(:,1:4),dist{2})']);
    end;
    K=zeros(size(D));
  else
    nd=size(c1{1},2)-1;
    switch char(dist{1})
      case 'coord2dist'
        Ds=eval([char(dist{1}),'(c1{1}(:,1:nd),c2{1}(:,1:nd))']);
        Dt=coord2dist(c1{1}(:,nd+1),c2{1}(:,nd+1));
      case 'coord2distRiver'
        Ds=eval([char(dist{1}),'(c1{1}(:,1:4),c2{1}(:,1:4),dist{2})']);
        Dt=coord2dist(c1{1}(:,nd+1),c2{1}(:,nd+1));
    end;
    K=zeros(size(Ds));
  end;
