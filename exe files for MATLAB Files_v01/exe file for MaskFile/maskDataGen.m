 ...
 existID = exist('maskFileName.txt', 'file');
    if (existID==2)
        ...
        if ext =='.shp'
            ...
            if length(c)>1                 
                errordlg(sprintf('Mask Error: Shape file has more than %s Polygons\n BMEGUI handles only single polygon',num2str(length(c))),'BMEGUI: Mask File Error');               
                return;                         
            end 
            N=find(isnan(c.X));
            ...
            if length(N)>1
                errordlg(sprintf('Mask Error: Shape file has %s (or more) Polygons\n BMEGUI handles only single polygon',num2str(length(N))),'BMEGUI: Mask File Error'); 
                return;
            else
                disp(sprintf('No of Polygons in your shape file = %d',length(N)));
            end 
            ...
            % Save X, Y data
            dlmwrite('maskX.txt',maskedge(:,1));
            dlmwrite('maskY.txt',maskedge(:,2));


        elseif ext =='.csv'           
            c =[];
            % Read .csv file if there is no header line
            try
                c=csvread(maskFilePath{1}); 
                disp('.csv file has no header line') ;                   
            end
            
            % Read .csv file if there is a HEADER LINE
            if isempty(c)
                c=csvread(maskFilePath{1},1); 
                disp('.csv file has header line') ;
            end
            
            % Finally we read .csv file  
                if size(c,2)>2
                    msg='.csv file should have only two columns (X, Y)';
                else
                    % Save X, Y data
                    dlmwrite('maskX.txt',c(:,1));
                    dlmwrite('maskY.txt',c(:,2));
                end
