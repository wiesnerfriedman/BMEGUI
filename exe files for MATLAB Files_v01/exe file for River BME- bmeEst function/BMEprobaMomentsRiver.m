function [moments,info]=BMEprobaMomentsRiver(ck,ch,cs,zh,softpdftype,nl,limi,probdens,covmodel,covparam,nhmax,nsmax,dmax,order,dist,options);
...
%   covmodel,covparam,nhmax,nsmax,dmax,order,dist,options);
%
% INPUT :
%
...
%%%%%% for Progress Bar ( P Jat: June, 2011) %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
txt = sprintf('Calculating BME Estimation\n\n Please Wait .......');
disp(txt);
...
if nargin<16,         % initialize options with default values
  options=BMEoptions;
  options1=1;
else
  options1=options(1);
...
for i=1:nk,
  ...
  [chlocal,zhlocal,dh,sumnhlocal]=neighbours(ck0,ch,zh,nhmax,dmax,dist);
  [cslocal,nllocal,limilocal,probdenslocal,ds,sumnslocal]=...
    probaneighbours(ck0,cs,softpdftype,nl,limi,probdens,nsmax,dmax,dist);
  ...
  if iscomputed==0
    ...
    Kk=coord2Kinterface(ck0,ck0,covmodel,covparam,dist);           % compute variance for estimation point
    Kh=coord2Kinterface(chlocal,chlocal,covmodel,covparam,dist);   % compute covariance matrix for hard data
    Ks=coord2Kinterface(cslocal,cslocal,covmodel,covparam,dist);   % compute covariance matrix for soft data
    Kk_h=coord2Kinterface(ck0,chlocal,covmodel,covparam,dist);     % compute cross-covariance vector estimation/hard data
    Kk_s=coord2Kinterface(ck0,cslocal,covmodel,covparam,dist);     % compute cross-covariance vector estimation/soft data
    Ks_h=coord2Kinterface(cslocal,chlocal,covmodel,covparam,dist); % compute cross-covariance matrix soft/hard data
