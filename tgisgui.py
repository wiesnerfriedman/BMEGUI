...
#################################################
#-------------------------FOR RIVER NETWORK PLOT P Jat: Jan 2, 2012 ----------------
##from pylab import *
##import numpy as np
from matplotlib.figure import Figure
from matplotlib.backends.backend_gtkagg import FigureCanvasGTKAgg as FigureCanvas
...
class TgisGui:
    def __init__(self,flgStdAlone,userConst,workDir=None,dataFile=None, dataFileRT=None):
        ...
        #-------------------------------------------- RIVER NETWORK ERROR CHECKS ----- P Jat: Jan 1, 2012 ------------------------
        ...
        if os.path.exists('stdPathRT.txt'):
            cmdChechRiver = 'checkRiverNetworkContinuityTests.exe ' + self.workingSpace
            retCode = os.system(cmdChechRiver)
            print retCode
            #if retCode==-1:#0:
            #    print
            #else:
            #    raise TgisError('Error in River Network test file execution')
            file = open('ErrorRiverNet.txt')
            line = file.readline()
            print '---------------------------------------'
            print line
            print '========================================'
            #------------------------ Intensive river network error checks ------------------------------------
            ...
            if line=='0.000000\n':
                #self.riverNetworkPlot('River network','green')
                ...
                RiverErrorMsg ='  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '               River Network Error :         ' +\
                                ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '  Error: Outlet location outside network.    '                    
                print 'Outlet location outside network '
                ...
                #gtk.main_quit()
                self.riverNetworkPlot('Error: Outlet location outside network','red')
                
            elif line=='2.000000\n':                
                RiverErrorMsg ='  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '               River Network Error :         ' +\
                                ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '  Error: Outlet at multiple reach endpoints.          '                       
                print 'Multiple outlet locations'
                riverNetworkerror = 'errorDial.exe ' + self.workingSpace
                #os.system(riverNetworkerror) # Error function in MATLAB 
                #raise TgisError(RiverErrorMsg)
                #gtk.main_quit()
                self.riverNetworkPlot('Error: Outlet at multiple reach endpoints','red')
                
            elif line=='3.000000\n':                
                RiverErrorMsg ='  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '               River Network Error :         ' +\
                                ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '  Error: Unknown outlet error.               '                       
                print 'unknown outlet error'
                riverNetworkerror = 'errorDial.exe ' + self.workingSpace
                #os.system(riverNetworkerror) # Error function in MATLAB 
                #raise TgisError(RiverErrorMsg)
                #gtk.main_quit()
                ...
                RiverErrorMsg ='  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '               River Network Error :         ' +\
                                ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '  Error: All River Reaches are not connected.' 
                                #'  See "River_Network_Error.png" figure ' + \
                                #'  at : ' + self.workingSpace                               
                print 'Broken network error'
                riverNetworkerror = 'errorDial.exe ' + self.workingSpace
                #os.system(riverNetworkerror) # Error function in MATLAB 
                #raise TgisError(RiverErrorMsg)
                #gtk.main_quit()
                self.riverNetworkPlotError('Error: Broken network','red')
                
            elif line=='5.000000\n':                    
                RiverErrorMsg ='  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '               River Network Error :         ' +\
                                ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' +\
                                '  Unknown river network error.               '                      
                print 'Unknown error identified in river network'
                riverNetworkerror = 'errorDial.exe ' + self.workingSpace
                #os.system(riverNetworkerror) # Error function in MATLAB 
                raise TgisError(RiverErrorMsg)
                gtk.main_quit()
    ...
    #----------------------------------- END: RIVER NETWORK CHECKS-------------------------------------------------
            
####        # Create first screen        
####        self.createScreen1()

    def riverNetworkPlot(self,title1,color1):
        ...
        ax.plot([x[len(x)-1]],[y[len(y)-1]], marker='s', markersize=10,color='red')
        ax.plot(x,y, '-g',lw=2)        
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_title(title1, color=color1)
        ax.legend(('Outlet','River network'),'upper right', shadow=True)
        ax.grid(True)        
    ...
    def riverNetworkPlotError(self,title1,color1):
        # Create toplevel window
        window11 = gtk.Window(gtk.WINDOW_TOPLEVEL)
        window11.set_title("BMEGUI: River network plot")
        window11.connect("destroy", gtk.main_quit)
        ...
        fig = Figure(figsize=(5,4), dpi=100)
        ax = fig.add_subplot(111)
        ax.plot([xx[len(xx)-1]],[yy[len(yy)-1]], marker='s', markersize=10,color='red')
        ax.plot(x,y, '-g',lw=2,label="test1")
        ax.plot(xx,yy, '-m',lw=2,label="test2")
        ax.set_xlabel('X')
        ax.set_ylabel('Y')
        ax.set_title(title1, color=color1)
        ax.legend(('Outlet','Connected network', 'Unconnected network'),'upper right', shadow=True)
        ax.grid(True)        
        canvas = FigureCanvas(fig)  # a gtk.DrawingArea
        ...
        # Create first screen
##        self.createScreen1()  % Jan 4, 2012 (call this function if there is no river network error
...
########            self.comboboxTest.set_active(0)
            ...
            if colNum < 8:           ##### original>      if colNum == 4 or colNum == 5: ####################################
            ...
            if not self.flgNewSession:
                ...
                if self.flgUseDataType:
                    ...
                    self.cmbVal1.set_active(listNumCol[4])    
...
##################################################################
    ...
    def selectMethod(self,*arg):
        ...
        self.chgCursor
        # Set quit message dialog object tree
        dialogName = "diagSelect1"   #==================Restored Old dialog box============= July 20,2011 ======
        selectDialogTree = gtk.glade.XML(gladeFile,dialogName)
        # Get widget objects
        selectDialog = selectDialogTree.get_widget(dialogName)
        ...
        #####len(self.dupLoc)
        #### #============Removed Newly developed error displaying dialog box and re-stored old one===== July 20,2011 ======
        #### #============Still new dialog box called 'diagSelect' will be used for diaplaying error types=== July 20,2011 ======
        ####
        ####        self.label512 = selectDialogTree.get_widget('label512')   #---------------- BME Solution Text --------- P Jat
...
##                # Eliminate duplicated measurement from hardened data               
    ...
    def goBox3(self,widget):
        ...
        try:
            ...
            ##----- Blocked by P Jat (Sept, 2011)- These 2 lines looks for ArcGIS and if no ArcGIS then block 'Point Layer file' button on Screen 3
            ##if self.flgStdAlone:
...
# P Jat blocked this section (Creat Point Layer File using ArcGIS (on screen 3)            
...
####            self.GP.AddField(lyrTableName,"Val","double")
            ...
            # --- Get user specified File_Name and Adress to save file -------
            save_CSVfile_name = self.file_browse(gtk.FILE_CHOOSER_ACTION_SAVE)            
            if (save_CSVfile_name != ""):
                #--- Use User defined file name and place to save file                
                (headCSV, tailCSV) = os.path.split(save_CSVfile_name)   # Split Path and File names head= PATH; tail = FILE_NAME

                f =open(save_CSVfile_name + '.csv','wt')
                #f = open('PointLayer.csv', 'wt')
                try:
                    writer = csv.writer(f)
                    writer.writerow( ('X', 'Y', 'T', 'Val') )
                    for i in range(len(expX)):
                        #writer.writerow( (i+1, chr(ord('a') + i), '08/%02d/07' % (i+1)) )
                        writer.writerow((expX[i], expY[i], expT[i], expVal[i]))
                finally:
                    f.close()

                strMsg = "CSV Point File (.csv):                          " +  tailCSV +  ".csv                                                            "+\
                         "                                                      has been created"
...
##---------------- P Jat (Sept, 2011): this section is a logical check if BMEGUI working with ArcGIS
##            # Deactivate button
##            if self.flgStdAlone:
        ...
        try:
            ...
            if (save_CSVfile_name != ""):
                ...
                try:
                    ...
                    for i in range(len(self.sptlGridX)):
                        #writer.writerow( (i+1, chr(ord('a') + i), '08/%02d/07' % (i+1)) )
                        writer.writerow((self.sptlGridX[i], self.sptlGridY[i], self.sptlSmMean[i]))
                finally:
                    f.close()
...
####            main()
    ...
    #---------------------------------------------------------------------------------------------
    #-------------------------------------------------------------------------------------@@@@----        
    # ---------------------P Jat:  Function to fit automatic covariance model to experimental cov              
    def automaticCovFit(self, widget):
        # Delete covariance data Files
        if os.path.exists('sptLag.txt'):
            os.remove('sptLag.txt')
        if os.path.exists('covValSpt.txt'):
            os.remove('covValSpt.txt')
        if os.path.exists('lagTmp.txt'):
            os.remove('lagTmp.txt')
        if os.path.exists('covValTmp.txt'):
            os.remove('covValTmp.txt')

        FILE = open('lagSpt.txt', "w", 0)
        for i in self.sptlLag:
            FILE.writelines(str(i)+ '\n')
            
        FILE = open('covValSpt.txt', "w", 0)
        for i in self.sptlCovariance:
            FILE.writelines(str(i)+ '\n')


        FILE = open('lagTmp.txt', "w", 0)
        for i in self.tempLag:
            FILE.writelines(str(i)+ '\n')

        FILE = open('covValTmp.txt', "w", 0)
        for i in self.tempCovariance:
            FILE.writelines(str(i)+ '\n')
            
        retCode = os.system('autoCovFitBMEGUI.exe ')
        print 'Best fit Covariance model found'
        
        reader = csv.reader(open('fittedCovModel.txt', "rb"))
        for covFitData in reader:
            print ''
        # covFitData[0] : CovModel number Spatial Cov Model
        # covFitData[1] : sill
        # covFitData[2] : Range for Spatial Cov model
        # covFitData[3] : CovModel number for Temporal Cov model
        # covFitData[4] : Range for Temporal Cov Model
        self.entCovPrm1.set_text(covFitData[1])
        self.entSptlCovPrm1.set_text(covFitData[2])
        self.entTempCovPrm1.set_text(covFitData[4])

        # Set best fit spatial covariance model
        if int(covFitData[0])==1:
            self.cmbSptlCovMdl1.set_active(self.chkModel('exponentialC'))
        elif int(covFitData[0])==2:
            self.cmbSptlCovMdl1.set_active(self.chkModel('gaussianC'))
        elif int(covFitData[0])==3:
            self.cmbSptlCovMdl1.set_active(self.chkModel('sphericalC'))

        # Set best fit Temporal covariance model
        if int(covFitData[3])==1:
            self.cmbTempCovMdl1.set_active(self.chkModel('exponentialC'))
        elif int(covFitData[3])==2:
            self.cmbTempCovMdl1.set_active(self.chkModel('gaussianC'))
        elif int(covFitData[3])==3:
            self.cmbTempCovMdl1.set_active(self.chkModel('sphericalC'))

        # Plot best found model automatically without clicking on 'Plot Model'
        self.plotMdl()
        ...
        if widget.get_active():
            ...
            if os.path.exists('mapX.txt'):
                ...
                if response == gtk.RESPONSE_OK:
                    ...
                    fileSelected=1
                    dialog.destroy()
                    if not os.path.exists('mapX.txt'):
                        self.lowWindow6.set_cursor(self.cursorWatch)
    ...
    def goBox6(self,widget):
        ...
        try:
            ...
            # P Jat: Added these line (if river net is analysing diasble Voronoi
            if os.path.isfile('stdPathRT.txt')==True:           # P Jat
                self.butCreateRaster.set_sensitive(0)           # P Jat (Jan 2015)
            else:
                print 'River networ is not being analyzed'
            #-----------------------------------------------
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if self.incMap ==1:
                ...
                file.close()
                plotBMEMap.hold(True)
                plotBMEMap.plot(xxmap,yymap,color='black',lw=1)                
            else:
                print 'Do not plot map'
            ...
            if self.incMask ==1:                         
                ...
                for line in file.xreadlines():
                    y.append(float(line.rstrip('\n')))
                file.close()
                plotBMEMap.fill(x,y,facecolor='w', edgecolor='white',alpha=1)               
            else:
                print #'Mask file does not exist'
...
##                    self.plotedNaNId_V.remove(int(newNumber))
    ...
    def plotBMEMap(self,widget):
        ...
        try:
            ...
            # Plot map
            if os.path.isfile('stdPathRT.txt')==True:           # P Jat
                estmap = plotBMEMap.scatter(smPtsX,smPtsY,20,smMean)           # P Jat (Jan 2015)
            else:
                estmap = plotBMEMap.pcolor(meshX,meshY,meshMean,cmap=get_cmap(self.textColor),shading='interp',vmin=minVal,vmax=maxVal)
            ...
            # Plot map
            if os.path.isfile('stdPathRT.txt')==True:               # P Jat
                varmap = plotBMEErr.scatter(smPtsX,smPtsY,20,smVar) # P Jat (Jan 2015)
            else:
                varmap = plotBMEErr.pcolor(meshX,meshY,meshVar,cmap=get_cmap(self.textColor),shading='interp',vmin=minVal,vmax=maxVal)
            figBMEErr.colorbar(mappable = varmap)
            plotBMEErr.hold(True)
            ...
            #if os.path.exists('maskY.txt'):
            if self.incMask ==1:
                plotBMEErr.fill(x,y,facecolor='w', edgecolor='white',alpha=1)
            else:
                print #'Mask file does not exist'

            if self.incMap ==1:
                plotBMEErr.plot(xxmap,yymap,color='black',lw=1)
            else:
...
##            tt =estPtsT
##            CICI =upperCI
##            for i in range(len(estPtsT),0,-1):
##                tt.append(estPtsT[i-1])
##                CICI.append(lowerCI[i-1])            
##            fig =p.figure(110)
##            p.fill(tt,CICI,facecolor='red')
##            p.hold(True)
##            #p.plot(estPtsT,estMean,'-b')
##            p.plot(estHardTime,estHardVal,'bo')           
##            p.show()
##            print len(tt)
                ...
                if flgMeanTab:
                    if int(plotId) in self.plotedNaNId_M:
                        ...
                        if self.incMask ==1:
                            ...
                            file.close()
                            plotObj.hold(True)
                            plotObj.fill(x,y,facecolor='w', edgecolor='white',alpha=1)
                        else:
                            print #'Mask file does not exist'
                        ...
                        if self.incMap ==1:
                            ...
                            file.close()
                            plotObj.hold(True)
                            plotObj.plot(xxmap,yymap,color='black',lw=1)
                            #-+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                            plotObj.set_xlim([min(xxmap),max(xxmap)])
                            plotObj.set_ylim([min(yymap),max(yymap)])
                        else:
                            print '' 
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        ...
                        titleString = 'BME Mean Estimates on ' + str(estTime) + \
                                      ' ' + self.tempUnit + ' ' + unitString
                        setPlotLabels(plotObj,xlabelString, ylabelString,titleString,self.GridStat)
...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    #---- Basically it Creates a .csv file containing, X, Y, BMEmean, BMEvar for specified time
    # NOTE: I am not changing 'createBMEPtLayer' to 'createBMEcsv' file just to avoid confusion
    #       since its is modified function not a completely new idea
    def createBMEPtLayer(self,widget):
        ...
        try:
          ...
          #----- Save X, Y, BME estimated Mean and Var as .CSV file( P Jat; June 2011)-------
            strHeader ='BME' 
            csvFileName = strHeader + plotId
            if os.path.exists('dataX_csv.txt'):
                os.remove('dataX_csv.txt')
            if os.path.exists('dataY_csv.txt'):
                os.remove('dataY_csv.txt')
            if os.path.exists('dataMean_csv.txt'):
                os.remove('dataMean_csv.txt')
            if os.path.exists('dataVar_csv.txt'):
                os.remove('dataVar_csv.txt')               

                
            myfile = open("dataX_csv.txt", "wb")
            for item in estPtsX:
                myfile.write(str(item)+',')               
            myfile.close()
            
            myfile = open("dataY_csv.txt", "wb")
            for item in estPtsY:
                myfile.write(str(item)+',')               
            myfile.close()
            
            myfile = open("dataMean_csv.txt", "wb")
            for item in estMean:
                myfile.write(str(item)+',')               
            myfile.close()
           
            myfile = open("dataVar_csv.txt", "wb")
            for item in estVar:
                myfile.write(str(item)+',')               
            myfile.close()
            ...
            ####            if os.path.exists(self.workingSpace +'/VectorData_files'):
            ####                print 'yes VectorData_files Directory exist'
            ####            else:
            ####                arcGridDir =os.mkdir(self.workingSpace+ '/VectorData_files')                
            ####            csvFileName = self.workingSpace +'/VectorData_files/'+ csvFileName            
            ####            cdmCSVWriter = 'csvFileWriter.exe '+ csvFileName


            if (save_CSVfile_name != ""):
                #--- Use User defined file name and place to save file
                cdmCSVWriter = 'csvFileWriter.exe '+save_CSVfile_name
                (headCSV, tailCSV) = os.path.split(save_CSVfile_name)   # Split Path and File names head= PATH; tail = FILE_NAME
                retCode = os.system(cdmCSVWriter)
                ##            strMsg = "CSV Point File (.csv):                          " + strHeader + plotId +  ".csv                                    "+\
                ##                     "                                                    have been created"
                strMsg = "CSV Point File (.csv):                          " +  tailCSV +  ".csv                                                            "+\
                         "                                                      has been created"
                self.dispRegMsg(strMsg)
    ...
    def createBMERasterLayer(self,widget):
        ...
        try:
            ...
            #-------Save X, Y, and BME estimated Mean and Var as ArcASCII file  ( P Jat; June 2011)----
            strHeader ='BME'
            ...
            save_ArcASCIIfile_name= self.file_browse(gtk.FILE_CHOOSER_ACTION_SAVE)

            if (save_ArcASCIIfile_name != ""):                
                cdmArcGridWrite = 'arcgridwrite.exe '+ save_ArcASCIIfile_name                
                ...
                (headArcASCII, tailArcASCII) = os.path.split(save_ArcASCIIfile_name)   # Split Path and File names head= PATH; tail = FILE_NAME
                strMsg = "Arc ASCII (.asc) files:       " + tailArcASCII + "MeanArcASCII" + ".asc                         "+\
                         tailArcASCII + "VarArcASCII" +  ".asc                         " + "                     have been created"
                self.dispRegMsg(strMsg)
                # Change cursor to None
            self.lowWindow6.set_cursor(None)
    ...
    #-------------- Get File name and place to save BME ArcASCII or .CSV file -----------------------
    ...
    #--------------END:  Get File name and place to save BME ArcASCII or .CSV file -----------------------
