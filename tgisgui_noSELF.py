...
class TgisGui:
    ...
    def __init__(self,flgStdAlone,userConst,workDir=None,dataFile=None):
        """Constructor"""
        self.numCurScreen = 0
    ...
    def createScreen1(self):
        ...
        try:
            ...
            if not self.flgNewSession:
                ...
                #self.cmbXX.set_active(listNumCol[4])          ##### P JAT #################
                #self.cmbYY.set_active(listNumCol[5])          ##### P JAT #################
    ...
    #------------------------------------ P Jat -------- New Def for BMEGUI Error Handling----------
    def BMEGUIerrorH(self, widget):
        ...
        jj=0        # pjat ---------------------------
        print dupLoc                                
        msgQuestion = 'Station ' + str(dupLoc[ii][jj]) + ' assigned to' # p jat ------------------------------
                    
        #BMEtable = '\n'
        self.dataFile = createDataFileReader(self.dataFileName)
        colNum = self.dataFile.getColName()
        BMEtable = '( '+ colNum[0] + ' , '+ colNum[1] +' )\n'
        for item in dupLoc[0][1]:
            i=i+1
            BMEtable = BMEtable + '( ' + str(item[0]) + ' , ' + str(item[1]) + ' ) \n'
            BMEavg1 = sdupLoc[0][1][0][0]+dupLoc[0][1][1][0]
            BMEavg2 = dupLoc[0][1][0][1]+dupLoc[0][1][1][1]
        msgQuestion = msgQuestion  + str(i) + ' different locations.' # There are ' + str(len(self.dupLoc)) + '\n such station(s).' #---- P jat-----------------------------
        ...
        BMEsol = ' To correct this error, BMEGUI will assign station ' + str(dupLoc[0][0]) + ' \n' + \
                 'to new location (' + str(BMEavg1/i) + ', ' + str(BMEavg2/i) + ' )'
        flgRemvLoc = self.selectMethod(msgQuestion,msgOpt1,msgOpt2, BMEsol, BMEtable)  #-------- P jat
    ...
    def selectMethod(self,*arg):
        ...
        self.label523.set_text('Error ' +  + ' of ' + )
        ...
        try:
            ...
            if not self.flgNewSession:
                ...
                if not self.flgUseDataType:
                    ...
                    listTempCmb = [self.cmbX,self.cmbY,self.cmbTime,\
                                   self.cmbData,self.cmbVal1,self.cmbVal2]
            ...
            if self.flgNewSession:
                ...
                if len(dupLoc) != 0:
                  ...
                  #-------------------- Original codes ------------------------------------------- P jat------
                  #  msgQuestion = 'Same station ID assigned to ' + \
                  #                'different locations. ' + \
                  #                'There are ' + str(len(self.dupLoc)) + \
                  #                ' such station(s). \n' + \
                  #                '(Example) \n' + \
                  #                'ID: ' + str(self.dupLoc[0][0]) + '\n' + \
                  #                'Location: '
                  #-------------------- Original codes end--------------------------------------------
                    i=0;        # pjat ----------------------------------------------------------------------------------------
                    ii=0        # pjat ---------------------------
                    ...
                    for item in dupLoc[0][1]:
                        i=i+1
                        BMEtable = BMEtable + '( ' + str(item[0]) + ' , ' + str(item[1]) + ' ) \n'
                        BMEavg1 = dupLoc[0][1][0][0]+dupLoc[0][1][1][0]
                        BMEavg2 = dupLoc[0][1][0][1]+dupLoc[0][1][1][1]
                   ...
                   # msgOpt2 = 'Quit Application and correct data manually'
                    ...
                    flgRemvLoc = self.selectMethod(msgQuestion,msgOpt1,msgOpt2, BMEsol, BMEtable)  #------------------ P jat
                    #flgRemvLoc = self.selectMethod2(msgQuestion,msgOpt1,msgOpt2) #------------- original-------------------------
                    if flgRemvLoc:
                ...
                if len(dupId) != 0:
                    msgQuestion = 'Location ( ' + str(dupId[0][0][0]) + ' ) are assigned to the ' + str(dupId[0][0][1]) +') \n' + 'ID: '  # P jat
                    flgRemvLoc = self.selectMethod(msgQuestion,msgOpt1,msgOpt2, BMEsol, BMEtable)
                ...
                if len(self.objAveHard.getX()) != len(self.objHardData.getX()):
                    ...
                    BMEsol = ' '        # P jat
                    BMEtable =''        # P Jat
                    flgRemvId = self.selectMethod(msgQuestion,msgOpt1,msgOpt2, BMEsol, BMEtable)               # P jat
                    if flgRemvId:
                        pass
                    else:
                        raise DummyError
